...
def read_records(file_path,cursor):
    ...
    with open(file_path,"r") as txt_file:
        for line in txt_file:
            ...
            data=line.split("$")
            print data
            sql="INSERT INTO report_links(REPORT_LINK_ID,REPORT_ID,RECORD_TYPE_ENG,RECORD_TYPE_FR,REPORT_LINK_NO) VALUES('%d','%d','%s','%s','%s')" %(int(data[0]),int(data[1]),data[2],data[3],data[4])
            cursor.execute(sql)
            counter+=1
    return counter
