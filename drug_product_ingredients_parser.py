...
def read_records(file_path,cursor):
    ...
    with open(file_path,"r") as txt_file:
        for line in txt_file:
            ...
            data=line.split("$")
            print data
            sql="INSERT INTO drug_product_ingredients(DRUG_PRODUCT_INGREDIENT_ID,DRUG_PRODUCT_ID,DRUGNAME,ACTIVE_INGREDIENT_ID,ACTIVE_INGREDIENT_NAME) VALUES('%d','%d','%s','%d','%s')" %(int(data[0]),int(data[1]),data[2],int(data[3]),data[4])
            cursor.execute(sql)
            counter+=1
    return counter
