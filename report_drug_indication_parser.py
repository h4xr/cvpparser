    ...
    with open(file_path,"r") as txt_file:
        for line in txt_file:
            ...
            if(data[2]==''):
                data[2]='0'
            print data
            sql="INSERT INTO report_drug_indication(REPORT_DRUG_ID,REPORT_ID,DRUG_PRODUCT_ID,DRUGNAME,INDICATION_NAME_ENG,INDICATION_NAME_FR) VALUES('%d','%d','%d','%s','%s','%s')" %(int(data[0]),int(data[1]),int(data[2]),data[3],data[4],data[5])
            cursor.execute(sql)
            counter+=1
    return counter
